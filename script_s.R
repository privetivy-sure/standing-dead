...
write.table(trees_consistent_L, 
            file = "trees_consistent_L.txt", 
            sep = "\t",          
            dec = ".",           
            row.names = FALSE, 
            quote = FALSE,       
...
# Project: Standing Deadwood Analysis
# ==============================================================================
...
# Revision needed:
#         - For a few LFLRP trees (cca 56) rows with missing life - to delete this row
#           Now the whole tree_id is deleting, which is wrong
# ------------------------------------------------------------------------------
...
duplicity_test <- trees_dead %>%
  # Filter only target sites where the issue occurs
  filter(composed_site_id %in% c(
    "VUK__1__Zofin__a", "VUK__1__Zofin__b", 
    "VUK__1__Zofin__c", "VUK__1__Zofin__d", 
    "VUK__15__Ranspurk__NA"
  )) %>%
  # Group by year, site, and tree/stem ID
  # (Note: Adjust column name if the tree ID column is named differently than tree_id)
  group_by(composed_site_id, inventory_year, tree_id, stem_id) %>%
  # Count occurrences of each combination
  summarise(pocet_zaznamu = n(), .groups = "drop") %>%
  # Filter only records that appear 2 or more times
  filter(pocet_zaznamu > 1)

# Console output
print(paste("Number of duplicate stems found:", nrow(duplicity_test)))
head(duplicity_test)
...
# ==============================================================================
# Revision is needed: Is dbh_group necessary?
#               -    Duplicates: Think over the use of plot_id2 for Zofin and Ranspurk
#               -    Moving this chapter to another place
# ------------------------------------------------------------------------------
...
# ==============================================================================
# 1. IDENTIFY TREES THAT NEVER EXCEEDED OR REACHED DBH 50
# ==============================================================================
tree_max_dbh <- trees_surv %>%
  group_by(tree_unique_id) %>%
  # Find the absolute maximum DBH achieved by each tree in its history
  summarize(max_dbh = max(dbh, na.rm = TRUE), .groups = "drop")

# Filter IDs where the maximum DBH is strictly below 50
ids_to_remove <- tree_max_dbh %>%
  filter(max_dbh < 50 | is.na(max_dbh)) %>%
  pull(tree_unique_id)

# 2. SAVE EXCLUDED TREES INTO A SEPARATE DATAFRAME
# Extract the full history of the small trees
trees_under_50 <- trees_surv %>%
  filter(tree_unique_id %in% ids_to_remove)

# 3. REMOVE THESE TREES FROM THE MAIN DATAFRAME
# Keep only trees whose ID is NOT in the removal list
trees_surv_thres <- trees_surv %>%
  filter(!tree_unique_id %in% ids_to_remove)
...
trees_small_joined <- trees_dead8 %>%
...
trees_dead9 <- bind_rows(trees_big_joined, trees_small_joined) %>%
  # Arrange chronologically to maintain historical sequence per tree stem
  arrange(tree_unique_id, inventory_year)


# ==============================================================================
# AGGREGATE PLOT-LEVEL INVENTORY TIMES SPANS
# ==============================================================================

plots_summary <- trees_dead9 %>%
  # Group by the unique plot ID and all its descriptive attributes to keep them in the output
  group_by(plot_unique_id, site_name, composed_site_id, plot_id2) %>%
  
  # Calculate min year, max year, and the difference (span) for each plot
  summarize(
    inventory_year_min = min(inventory_year, na.rm = TRUE),
    inventory_year_max = max(inventory_year, na.rm = TRUE),
    year_difference    = max(inventory_year, na.rm = TRUE) - min(inventory_year, na.rm = TRUE),
